...
_applyInsideValueY() {
  ...
  async _maybeUpdateStats() {
    ...
    try {
      ...
      console.warn("Andy Sensor Card v1.0.0: history fetch failed (REST)", e);
...
if (b.style === "fan" || b.style === "heatpump") {
  ...
  return html`
    <div class="${symCls}" style="${symStyleVars}" data-badge-id="${b.id}" @click=${(ev) => this._onBadgeTap(ev, b)}>
...
if (b.show_slider) {
  ...
  return html`
    <div class="${sliderCls}" style="${sliderStyle}" @click=${(ev) => this._onBadgeTap(ev, b)}>
...
return html`
  <div class="${cls}" style="${styleVars}" data-badge-id="${b.id}" @click=${(ev) => this._onBadgeTap(ev, b)}>
            ${(recycle) ? html`
              ${(() => {
...
.asc-badge .recycleIcon .arc{
  ...
  stroke-linejoin: round;
      ...
      .asc-badge.flowAnim{ position: relative; }
